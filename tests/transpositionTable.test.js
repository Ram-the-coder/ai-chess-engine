...
const hash2 = transpositionTable.computeZobristHash(board2);
game2.move('Nxe5');
...
const hash5 = transpositionTable.computeZobristHash(board5);
...
const hash6 = transpositionTable.computeZobristHash(board6);
...
const getHashTestCases = [
	{
		...
		expected: transpositionTable.piecePositionValue[5][52]
	},
	{
		test: [{type: 'p', color: 'w'}, 4, 4],
		expected: transpositionTable.piecePositionValue[5][36]
	},
	{
		test: [{type: 'n', color: 'b'}, 2, 2],
		expected: transpositionTable.piecePositionValue[7][18]	
	},
	{
		test: [{type: 'n', color: 'b'}, 3, 4],
		expected: transpositionTable.piecePositionValue[7][28]	
	},
	{
		test: [{type: 'k', color: 'w'}, 7, 4],
		expected: transpositionTable.piecePositionValue[4][60]		
	},
	{
		test: [{type: 'r', color: 'w'}, 7, 0],
		expected: transpositionTable.piecePositionValue[0][56]		
	}
]

getHashTestCases.forEach(testCase => {
	test('Get has of piece', () => {
		expect(transpositionTable.getHashValueOfPiece(...testCase.test)).toBe(testCase.expected);
	})
})

const recomputeTestCases = [
	{
		test: [0, board, 'e4', true],
		expected: transpositionTable.getHashValueOfPiece({type: 'p', 'color': 'w'}, 6, 4) ^ transpositionTable.getHashValueOfPiece({type: 'p', 'color': 'w'}, 4, 4)
	},
	{
		test: [hash2, board2, 'Nxe5', false],
		expected: transpositionTable.computeZobristHash(game2.board())
	},
	{
		test: [hash3, board3, 'Rg8', false],
		expected: transpositionTable.computeZobristHash(game3.board())
	},
	{
		test: [hash4, board4, 'O-O-O', true],
		expected: transpositionTable.computeZobristHash(game4.board())
	},
	{
		test: [hash5, board5, 'Ncxe7', false],
		expected: transpositionTable.computeZobristHash(game5.board())
	},
	{
		test: [hash6, board6, 'Rhh6', true],
		expected: transpositionTable.computeZobristHash(game6.board())
	},

]
recomputeTestCases.forEach(testCase => {
	test('Recompute Zob Hash ' + testCase.test[2], () => {
		const hash = transpositionTable.recomputeZobristHash(...testCase.test);
